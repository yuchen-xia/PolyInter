...
def get_transformation_ego2neb(ego_lidar_pose, neb_lidar_pose_lst, max_cav):
    """
    Get pair-wise transformation matrix.
    
    """
    t_list = [x1_to_x2(ego_lidar_pose, neb_lidar_pose) for neb_lidar_pose in neb_lidar_pose_lst]
    pairwise_t_matrix = np.tile(np.eye(4), (1, max_cav, 1, 1)) # (1, L, 4, 4)
    for j in range(len(t_list)):
        pairwise_t_matrix[0, j] = t_list[j]
