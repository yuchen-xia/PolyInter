...
from opencood.utils.transformation_utils import x1_to_x2, x_to_world, get_pairwise_transformation, get_transformation_ego2neb
...
def getIntermediateheterpairpolyinterstage1FusionDataset(cls):
    """
    cls: the Basedataset.
    """
    class IntermediateheterPolyInterStage1FusionDataset(cls):
        def __init__(self, params, visualize, train=True):
            ...
            # print("1111111111111",self.sensor_type_dict)
            self.reinitialize()
        ...
        def __getitem__(self, idx):
            ...
            if self.visualize or self.kd_flag:
                projected_lidar_stack_m2 = []
                projected_lidar_stack_m3 = []
            ...
            for _i, cav_id in enumerate(cav_id_list):
                ...
                # modality_name = selected_cav_base['modality_name']
                if cav_id == ego_id:
                    ego_lidar_pose = selected_cav_base['params']['lidar_pose']
                    # ego m1
                    sensor_type = self.sensor_type_dict[self.ego_modality]
                    selected_cav_processed = self.get_item_single_car(
                        selected_cav_base,
                        ego_cav_base,
                        self.ego_modality)
                    if sensor_type == 'lidar':
                        ego_input_list.append(selected_cav_processed[f"processed_features_{self.ego_modality}"])
                    else:
                        ego_input_list.append(selected_cav_processed[f"image_inputs_{self.ego_modality}"])
                    if self.visualize:
                        projected_lidar_stack_m2.append(
                            selected_cav_processed['projected_lidar'])
                        projected_lidar_stack_m3.append(
                            selected_cav_processed['projected_lidar'])
  
                else:
                    neb_lidar_pose_lst.append(selected_cav_base['params']['lidar_pose'])
                    # m2
                    sensor_type = self.sensor_type_dict['m2']
                    selected_cav_processed = self.get_item_single_car(
                        selected_cav_base,
                        ego_cav_base,
                        'm2')
                    if sensor_type == 'lidar':
                        neb_input_list.append(selected_cav_processed[f"processed_features_m2"])
                    else:
                        neb_input_list.append(selected_cav_processed[f"image_inputs_m2"])
                    if self.visualize:
                        projected_lidar_stack_m2.append(
                            selected_cav_processed['projected_lidar'])
                    
                    # m3
                    sensor_type = self.sensor_type_dict['m3']
                    selected_cav_processed = self.get_item_single_car(
                        selected_cav_base,
                        ego_cav_base,
                        'm3')
                    if sensor_type == 'lidar':
                        neb2_input_list.append(selected_cav_processed[f"processed_features_m3"])
                    else:
                        neb2_input_list.append(selected_cav_processed[f"image_inputs_m3"])
                        
                    if self.visualize:
                        projected_lidar_stack_m3.append(
                            selected_cav_processed['projected_lidar'])
            
            assert cav_num == len(ego_input_list) + len(neb_input_list)
            assert len(ego_input_list) == 1
            assert len(neb_input_list) == len(neb2_input_list) == len(neb_lidar_pose_lst)
            
            
            if len(neb_input_list)==0:
                # 邻车没有就补一个自车
                ...
                # 邻车有，把自车复制到跟邻车数量一致。
                for i in range(1, len(neb_input_list)):
                    ego_input_list.append(copy.deepcopy(ego_input_list[0]))
            
            assert len(neb_input_list) == len(neb2_input_list) == len(neb_lidar_pose_lst)
            
                
            ego2neb_t_matrix = get_transformation_ego2neb(ego_lidar_pose, neb_lidar_pose_lst, max_cav=self.max_cav)
            # ego2neb_t_matrix: [1, max_cav, 4, 4]，代表自车到neb_input_lst中的车的t_matrix
            ...
            # ego, m1
            if self.sensor_type_dict[self.ego_modality] == 'lidar':
                merged_feature_dict = merge_features_to_dict(ego_input_list)
                processed_data_dict['ego'].update({f'input_ego': merged_feature_dict})
            else:
                merged_feature_dict = merge_features_to_dict(ego_input_list, merge='stack')
                processed_data_dict['ego'].update({f'input_ego': merged_feature_dict})
            
            # m2
            if self.sensor_type_dict['m2'] == 'lidar':
                merged_feature_dict = merge_features_to_dict(neb_input_list)
                processed_data_dict['ego'].update({f'input_neb': merged_feature_dict})
            else:
                merged_feature_dict = merge_features_to_dict(neb_input_list, merge='stack')
                processed_data_dict['ego'].update({f'input_neb': merged_feature_dict})

            # m3 
            if self.sensor_type_dict['m3'] == 'lidar':
                merged_feature_dict = merge_features_to_dict(neb2_input_list)
                processed_data_dict['ego'].update({f'input_neb2': merged_feature_dict})
            else:
                merged_feature_dict = merge_features_to_dict(neb2_input_list, merge='stack')
                processed_data_dict['ego'].update({f'input_neb2': merged_feature_dict})
            ...
            processed_data_dict['ego'].update(
                ...
                'ego2neb_t_matrix': ego2neb_t_matrix,
                'lidar_poses_clean': lidar_poses_clean,
                'lidar_poses': lidar_poses})


            if self.visualize:
                processed_data_dict['ego'].update({'origin_lidar_m2':
                    np.vstack(
                        projected_lidar_stack_m2)})
                processed_data_dict['ego'].update({'origin_lidar_m3':
                    np.vstack(
                        projected_lidar_stack_m3)})
        ...
        def collate_batch_train(self, batch):
            ...
            choices = ['k', 'v']
            selected_char = random.choice(choices) #每个batch分配'k'或'v'
            # selected_char = 'v'
            ...
            for i in range(len(batch)):
                ...
                inputs_list_ego.append(ego_dict["input_ego"])
                if selected_char == 'k':
                    inputs_list_neb.append(ego_dict['input_neb'])
                else:
                    inputs_list_neb.append(ego_dict['input_neb2'])
                ...
                ego2neb_t_matrix_lst.append(ego_dict['ego2neb_t_matrix'])
            ...
            # ego m1   
            if self.sensor_type_dict[self.ego_modality] == "lidar":
                merged_feature_dict = merge_features_to_dict(inputs_list_ego)
                processed_lidar_torch_dict = eval("self.pre_processor_m1").collate_batch(merged_feature_dict)
                output_dict['ego'].update({f'inputs_ego': processed_lidar_torch_dict})
            else:
                merged_image_inputs_dict = merge_features_to_dict(inputs_list_ego, merge='cat')
                output_dict['ego'].update({f'inputs_ego': merged_image_inputs_dict})


            # neb m2/m3
            if selected_char == 'k':
                if self.sensor_type_dict['m2'] == "lidar":
                        merged_feature_dict = merge_features_to_dict(inputs_list_neb)
                        processed_lidar_torch_dict = eval("self.pre_processor_m2").collate_batch(merged_feature_dict)
                        output_dict['ego'].update({f'inputs_neb': processed_lidar_torch_dict})
                else:
                    merged_image_inputs_dict = merge_features_to_dict(inputs_list_neb, merge='cat')
                    output_dict['ego'].update({f'inputs_neb': merged_image_inputs_dict})
            elif selected_char == 'v':
                if self.sensor_type_dict['m3'] == "lidar":
                        merged_feature_dict = merge_features_to_dict(inputs_list_neb)
                        processed_lidar_torch_dict = eval("self.pre_processor_m3").collate_batch(merged_feature_dict)
                        output_dict['ego'].update({f'inputs_neb': processed_lidar_torch_dict})
                else:
                    merged_image_inputs_dict = merge_features_to_dict(inputs_list_neb, merge='cat')
                    output_dict['ego'].update({f'inputs_neb': merged_image_inputs_dict})
            ...
            ego2neb_t_matrix = torch.from_numpy(np.array(ego2neb_t_matrix_lst))
            ...
            output_dict['ego'].update({'selected_char': selected_char})
            return output_dict


        def collate_batch_test_helper(self, batch):
            # Intermediate fusion is different the other two
            output_dict = {'ego': {}}
            ...
            selected_char = 'k' # 一阶段模型文件测试时，selected_char用于设置邻车类型
            ...
            for i in range(len(batch)):
                ...
                if self.visualize:
                    if selected_char == 'k':
                        origin_lidar.append(ego_dict['origin_lidar_m2'])
                    else:
                        origin_lidar.append(ego_dict['origin_lidar_m3'])
        ...
        def collate_batch_test(self, batch):
            ...
            if batch[0] is None:
                return None
            output_dict = self.collate_batch_test_helper(batch)
    ...
    return IntermediateheterPolyInterStage1FusionDataset
