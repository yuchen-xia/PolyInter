...
class PolyInterStage2Loss(nn.Module):
    def __init__(self, args):
        super(PolyInterStage2Loss, self).__init__()
    ...
    def forward(self, output_dict, target_dict, suffix=""):
        ...
        # key 写的很乱不知道对不对，key是specific prompt的单独输出。输出个数跟batchsize不一样，而是跟邻车数量一样
        key_lens = output_dict['record_len']-1
        key_lens[key_lens==0] += 1 # 数据处理的时候没有邻车就补了一个自车的数据，这里加上
        ## 下面把对应标签全部按照各个场景中的车辆数量复制一下
        ...
        total_loss += reg_loss + cls_loss + reg_loss_key + cls_loss_key
